...
- **Embedded terminal emulator in the library** — the library exposes the integration *seam* (`./terminal` font/grid math, `PaneSession` per-pane state machine, `TerminalSink` interface, `./connectors/bridge` adapters) so any DOM emulator (xterm.js, Kitty, Zed) can wire in without re-implementing the glue. It does NOT ship its own emulator and never depends on xterm.js or any UI library. — *the seam is in scope; the renderer is not*
