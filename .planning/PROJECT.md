...
- **Terminal emulation in the library proper** (`src/`) — the realized terminal-integration layer ships as the separate `@promptctl/pane-terminal` workspace package (`PaneStream` / `TerminalSink` / `XtermSink` / `<PaneTerminal>`; design in `design-docs/pane-session-v2.md`), keeping the protocol library itself UI-agnostic with zero UI deps. — *consumers integrate however they want; the library stays minimal*
